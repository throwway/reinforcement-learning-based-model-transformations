...
public class MOContainerSearch {

   protected static final String CASE = "3S_8C";

   protected static final String INITIAL_MODEL = "model/" + CASE + ".xmi";

   protected static final boolean PRINT_POPULATIONS = true;
   protected static final String PRINT_DIRECTORY = "output/populations/" + CASE;
   ...
   public static void main(final String... args) {
      initialization();
      final MOContainerSearch search = new MOContainerSearch();
   ...
   protected final String[] unitsToRemove = new String[] { "container::containerModule::retrieveNonLastFromStack",
         ...
         "container::containerModule::relocateNonLastOnStackToEmptyStack", "container::containerModule::RelocateRandom",
         "container::containerModule::relocateNonLastOnStackToNonEmptyStack",
         "container::containerModule::RetrieveLooped", "container::containerModule::relocateLastOnStackToNonEmptyStack",
         "container::containerModule::checkIsLast", "container::containerModule::checkIsNotLast",
         "container::containerModule::checkTargetStackEmpty", "container::containerModule::RelocateLastOnStack",
         "container::containerModule::checkTargetStackNotEmpty", "container::containerModule::Relocate",
         "container::containerModule::Retrieve", "container::containerModule::RelocateNonLastOnStack",
   ...
   protected IFitnessDimension<TransformationSolution> _createObjective_1(
         final TransformationSearchOrchestration orchestration) {
      return new AbstractEGraphFitnessDimension("BlockingFactor",
   ...
   protected IFitnessDimension<TransformationSolution> _createObjective_2(
         final TransformationSearchOrchestration orchestration) {
      return new AbstractEGraphFitnessDimension("TravelDistance",
            at.ac.tuwien.big.moea.search.fitness.dimension.IFitnessDimension.FunctionType.Minimum) {
         @Override
         protected double internalEvaluate(final TransformationSolution solution) {
            final EGraph graph = solution.execute();
   ...
   protected IFitnessDimension<TransformationSolution> _createObjective_3(
         final TransformationSearchOrchestration orchestration) {
      return new AbstractEGraphFitnessDimension("Container-Index",
   ...
   protected double _createObjectiveHelper_2(final TransformationSolution solution, final EGraph graph,
         final EObject root) {
      return ContainerUtils.getTravelDistance((ContainerModel) root);
   }
   ...
   protected SearchExperiment<TransformationSolution> createExperiment(
      ...
      if(PRINT_POPULATIONS) {
         ...
         // final File file = new File(PRINT_DIRECTORY + "/out.txt");
         // PrintStream printStreamToFile;
         // try {
   ...
   protected TransformationSearchOrchestration createOrchestration(final String initialGraph,
      ...
      final Map<IEnvironment.Type, IEnvironment<TransformationSolution>> env = envBuilder.build();
      ...
      final double[] w = { 1.0, 1.0, 1.0 };

      orchestration.addAlgorithm("Chebycheff", rl.createChebyshevQLearner(w, 0.01, LocalSearchStrategy.NONE, 1, .9, 1.0,
            true, (1.0 - .05) / maxEvaluations, 0.05, "CH-CRA-" + CASE, 10, 0, null, null, false));

      orchestration.addAlgorithm("PQ-HV",
            rl.createParetoQLearner(LocalSearchStrategy.NONE, 1, .9, EvaluationStrategy.HYPERVOLUME, 1.0, true,
                  (1.0 - .05) / maxEvaluations, 0.05, "PQ-HV-" + CASE, 10, 0, null, null, false));

      orchestration.addAlgorithm("PQ-PO",
            rl.createParetoQLearner(LocalSearchStrategy.NONE, 1, .9, EvaluationStrategy.PARETO, 1.0, true,
                  (1.0 - .05) / maxEvaluations, 0.05, "PQ-PO-" + CASE, 10, 0, null, null, false));
      //
      // orchestration.addAlgorithm("PQ-PO-Local2",
      // rl.createParetoQLearner(LocalSearchStrategy.NONE, 1, .9, EvaluationStrategy.PARETO, 1.0, true,
      // (1.0 - .05) / maxEvaluations, 0.05, "PQ-PO-" + CASE, 10, 0, null, null, false));

      orchestration.addAlgorithm("Tournament", rl.createTournamentQLearner(LocalSearchStrategy.NONE, 1, .9, 1.0, true,
            (1.0 - .05) / maxEvaluations, 0.05, "T-" + CASE, 10, 0, null, null, false));

      // orchestration.addAlgorithm("Tournament-Local2", rl.createTournamentQLearner(LocalSearchStrategy.NONE, 1, .9,
      // 1.0,
      // true, (1.0 - .05) / maxEvaluations, 0.05, "T-CRA" + CASE, 10, 0, null, null, false));

      orchestration.addAlgorithm("NSGAII-2",
            moea.createNSGAII(new TournamentSelection(2), new OnePointCrossover(1.0),
                  new TransformationPlaceholderMutation(0.1),
                  new TransformationVariableMutation(orchestration.getSearchHelper(), 0.05)));
