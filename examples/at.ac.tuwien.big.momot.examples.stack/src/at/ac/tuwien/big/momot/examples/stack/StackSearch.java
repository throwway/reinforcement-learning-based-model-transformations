...
public class StackSearch {
   ...
   protected static final String CASE = "model_five_stacks";

   private static final int SOLUTION_LENGTH = 100;
   ...
   public static void main(final String[] args) throws IOException {
      ...
      search.addAlgorithm("QLearningExplore", rl.createSingleObjectiveExploreQLearner(10, 0.9, 1.0, true,
            (1.0 - .05) / MAX_EVALUATIONS, 0.05, "QExplore-" + CASE, 10, 0, null, null, false));

      search.addAlgorithm("QLearning", rl.createSingleObjectiveQLearner(0.9, 1.0, true, (1.0 - .05) / MAX_EVALUATIONS,
            0.05, "Q-" + CASE, 10, 0, null, null, false));
      ...
      if(PRINT_POPULATIONS) {
         ...
         experiment.addProgressListener(
               new CurrentNondominatedPopulationPrintListener(PRINT_DIRECTORY, algoNames, NR_RUNS, 10));
      }
   ...
   private List<ITransformationVariable> getOptimalSolutionForAlgorithm(
      ...
      for(final Entry<SearchExecutor, List<NondominatedPopulation>> entry : experiment.getResults().entrySet()) {
         ...
         StackSearch.saveModel(Paths.get(dir, algorithmName + "_model_" + modelFilename + ".xmi").toString(),
               optimalTS);
         return optimalTS.getVariablesAsList(false);
