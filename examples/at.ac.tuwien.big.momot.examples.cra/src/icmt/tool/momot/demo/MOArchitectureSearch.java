...
public class MOArchitectureSearch {

   protected static final String CASE = "TTC_InputRDG_D";

   protected static final int SOLUTION_LENGTH = 160;
   private static final String INPUT_MODEL = "model/" + CASE + ".xmi";
   ...
   public static void main(final String... args) {
      ...
      final MOArchitectureSearch search = new MOArchitectureSearch();
   ...
   public final int MAX_EVALUATIONS = 40000;

   protected final int NR_RUNS = 10;
   ...
   protected TransformationSearchOrchestration createOrchestration(final String initialGraph,
      ...
      // orchestration.getFitnessFunction().removeObjective("SolutionLength");

      final EnvironmentBuilder<TransformationSolution> envBuilder = new EnvironmentBuilder<>(
            createFitnessFunction(orchestration));
      ...
      final double[] w3 = { .5, .5, .5 };

      orchestration.addAlgorithm("Chebycheff-10", rl.createChebyshevQLearner(w3, 0.01, LocalSearchStrategy.GREEDY, 10,
            .9, 1.0, true, (1.0 - .05) / MAX_EVALUATIONS, 0.05, "CH-" + CASE, 10, 0, null, null, false));

      orchestration.addAlgorithm("PQ-HV-Local10",
            rl.createParetoQLearner(LocalSearchStrategy.GREEDY, 10, .9, EvaluationStrategy.HYPERVOLUME, 1.0, true,
                  (1.0 - .05) / MAX_EVALUATIONS, 0.05, "PQ-HV-" + CASE, 10, 0, null, null, false));

      orchestration.addAlgorithm("PQ-PO-Local10",
            rl.createParetoQLearner(LocalSearchStrategy.GREEDY, 10, .9, EvaluationStrategy.PARETO, 1.0, true,
                  (1.0 - .05) / MAX_EVALUATIONS, 0.05, "PQ-PO-" + CASE, 10, 0, null, null, false));

      orchestration.addAlgorithm("Tournament-Local10", rl.createTournamentQLearner(LocalSearchStrategy.GREEDY, 10, .9,
            1.0, true, (1.0 - .05) / MAX_EVALUATIONS, 0.05, "T-" + CASE, 10, 0, null, null, false));
