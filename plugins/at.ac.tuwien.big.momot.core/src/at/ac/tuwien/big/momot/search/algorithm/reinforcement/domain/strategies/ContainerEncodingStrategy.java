...
public class ContainerEncodingStrategy<S extends Solution> extends AbstractEncodingStrategy<S> {
   ...
   public INDArray[] encodeSolution(final S solution) {
      ...
      final INDArray[] encodingArrs = new INDArray[model.getStack().size()];
   ...
   public ComputationGraph getDDQNArchitecture(final S solution, final double learningRate,
      ...
      final int[] nHiddenDiscUnits = { 300 };
      // final int[] nHiddenConUnits = {256, 128};
      ...
      /*
       * final String blockingLayer = String.format("disc_in_%d", model.getStack().size());
      ...
      confBuild.addLayer("con_1", new DenseLayer.Builder().activation(Activation.LEAKYRELU).nOut(300).build(),
            hiddenDiscOutLayers);
      ...
      confBuild.addLayer("action", new OutputLayer.Builder(LossFunctions.LossFunction.L2)
            .nOut(this.getActionSpace(solution)).activation(Activation.IDENTITY).build(), "con_1");
      ...
      for(int i = 0; i < this.getActionSpace(solution); i++) {
         ...
         if(model.getStack().get(fromStack - 1).getTopContainer() == null
               || model.getStack().get(fromStack - 1).getTopContainer() != null
                     && model.getStack().get(fromStack - 1).getTopContainer().getOnTopOf() == null
                     && model.getStack().get(toStack - 1).getTopContainer() == null) {
            legalActions.remove(Integer.valueOf(i));
         }
      }
   ...
   @Override
   public String getModelVisualization(final S solution) {
