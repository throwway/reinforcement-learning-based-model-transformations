...
public class TransformationSolution extends SearchSolution implements Comparable<TransformationSolution> {
   ...
   private static Map<String, Object> extractParameters(final RuleApplication... raps) {
   ...
   public static TransformationSolution removePlaceholdersKeepUnitApplicationAssignment(
         final TransformationSolution solution) {
      final List<ITransformationVariable> nonPlaceholderVariables = new ArrayList<>();
      for(final ITransformationVariable variable : solution.getVariables()) {
         if(variable != null && !(variable instanceof IPlaceholderVariable)) {
            if(variable instanceof UnitApplicationVariable
                  && ((UnitApplicationVariable) variable).getAppliedRules().size() > 0) {
               final Map<String, Object> paramValues = TransformationSolution.extractParameters(
                     ((UnitApplicationVariable) variable).getAppliedRules().toArray(new RuleApplication[0]));
               final UnitApplicationVariable rav = (UnitApplicationVariable) variable.copy();
               for(final Parameter param : rav.getUnit().getParameters()) {
                  rav.setParameterValue(param, paramValues.get(param.getName()));
               }
               nonPlaceholderVariables.add(rav);
            } else {
               nonPlaceholderVariables.add(variable.copy());
            }
         }
   ...
   public TransformationSolution(final EGraph sourceGraph, final int numberOfVariables, final int numberOfObjectives,
      ...
      for(int i = 0; i < numberOfVariables; i++) {
         this.setVariable(i, new TransformationPlaceholderVariable());
      }
   }
   ...
   public List<ITransformationVariable> getVariablesAsList(final boolean removePlaceholderVars) {
      if(removePlaceholderVars) {
         return new ArrayList<>(Arrays.asList(variables)).stream().filter(v -> !(v instanceof IPlaceholderVariable))
