...
public class SearchHelper {
   ...
   public SearchHelper(final Engine engine, final ModuleManager moduleManager) {
      this.engine = engine;
   ...
   public TransformationSolution appendParticularVariableByAction(
         final IEncodingStrategy<TransformationSolution> encoder, final TransformationSolution solution,
         final int action) {
      ...
      final FixedRuleApplicationStrategy applicationStrategy = encoder.getFixedUnitApplicationStrategy(solution,
            action);
   ...
   public TransformationSolution appendVariables(final TransformationSolution solution,
         final List<IApplicationState> applicationStates) {
      ...
      for(final IApplicationState unitApplication : applicationStates) {
   ...
   public TransformationSolution createEmptyTransformationSolution(final EGraph g, final int solutionLength,
         final int nrObjectives, final int nrConstraints) {
      final TransformationSolution solution = new TransformationSolution(g, solutionLength, nrObjectives,
            nrConstraints);
      for(int i = 0; i < solutionLength; i++) {
         solution.setVariable(i, new TransformationPlaceholderVariable());
      }
      solution.setEqualityHelper(new DefaultEObjectEqualityHelper());
      return solution;
   }
   ...
   public ModuleManager getModuleManager() {
      return moduleManager != null ? moduleManager : getSearchOrchestration().getModuleManager();
   }
