...
public class NegotiatedWLearning<S extends Solution> extends AbstractMOTabularRLAgent<S> {
   ...
   public List<IApplicationState> epsGreedyDecision() {
      ...
      if(rng.nextDouble() >= this.eps) {
         ...
         int maxAgentIdx = rng.nextInt(this.environment.getFitnessComparators().size());
         double maxWeight = 0;
         ...
         do {
            int curAgentIdx = 0;
            converged = true;
            for(final IFitnessComparator<?, Solution> element : this.environment.getFitnessComparators()) {
   ...
   protected void iterate() {
      ...
      if(verbose && this.iterations % 5000 == 0) {
